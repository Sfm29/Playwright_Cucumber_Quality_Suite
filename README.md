...
- **The accessibility suite maintains a small, explicit, documented allowlist (`KNOWN_THIRD_PARTY_ISSUES` in `accessibility.steps.ts`) for two pre-existing issues on the site**: the footer "subscribe" button and the products-page search button are both icon-only with no accessible name (`button-name` violation). I don't control this site's markup, so "fix the bug" isn't on the table — but silently lowering the bar site-wide isn't either. The allowlist is scoped to those exact two selectors under that exact rule, so a *new* `button-name` violation anywhere else on the page still fails the build. This is the same shape of trade-off a real team hits constantly when testing a legacy page or a third-party integration they can't change on demand.
